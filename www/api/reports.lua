...
if report == 'cashflow' then
    ...
    for i = months - 1, 0, -1 do
        ...
        result[#result+1] = {
            ...
            income  = tonumber(inc[1].s) or 0,
            expense = tonumber(exp[1].s) or 0,
        }
    end
    util.ok(result)
...
elseif report == 'forecast' then
    ...
    local balance = tonumber(total_bal[1].s) or 0
    ...
    local pending_obls = G.query(
        ...
        u.id, now, ts_end)
    local pending_cost = tonumber(pending_obls[1].s) or 0

    local expected_inc = G.query(
        'SELECT COALESCE(SUM(amount),0) as s FROM transactions '..
        'WHERE user_id=? AND type="income" AND is_pending=1 AND date>=? AND date<?',
        u.id, now, ts_end)
    local income_pending = tonumber(expected_inc[1].s) or 0
    ...
    for d = 0, days do
        ...
        local day_obls = G.query(
            ...
            'WHERE user_id=? AND status="pending" AND due_date>=? AND due_date<?',
            u.id, ts, ts + 86400)
        running = running - (tonumber(day_obls[1].s) or 0)
        points[#points+1] = {
